...
exports.getUserByEmail = async (req, res) => {
    ...
    try {
        ...
        // Get user.
        const user = await User.findOne({ email: emailParam }).exec();
        if (!user) {
...
exports.updateUser = async (req, res) => {
    ...
    try {
        ...
        // Get user.
        const user1 = await User.findOne({ email: emailParam }).exec();
        if (!user1) {
            return res.status(404).json({ message: `User not found.` });
        }
        ...
        const user2 = await User.findByIdAndUpdate(userId, userParam, {
